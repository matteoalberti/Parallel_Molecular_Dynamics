...
{
    int i;
    double dmvsq2e = 1/mvsq2e;

    /* first part: propagate velocities by half and positions by full step */
    for (i=0; i<sys->natoms; ++i) {
        sys->vx[i] += 0.5*sys->dt *dmvsq2e * sys->fx[i] / sys->mass;
        sys->vy[i] += 0.5*sys->dt *dmvsq2e * sys->fy[i] / sys->mass;
        sys->vz[i] += 0.5*sys->dt *dmvsq2e * sys->fz[i] / sys->mass;
